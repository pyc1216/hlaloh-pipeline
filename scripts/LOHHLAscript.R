...
#lib_loc = '/home/ypu/2-Soft/miniconda3/envs/hemecdx/lib/R/library'
library(optparse) #, lib.loc=lib_loc)
option_list = list(
...
library(seqinr) #, lib.loc=lib_loc) #) #, quietly = TRUE)
library(Biostrings) #, lib.loc=lib_loc) #, quietly = TRUE)
library(beeswarm) #, lib.loc=lib_loc) #, quietly = TRUE)
library(zoo) #, lib.loc=lib_loc) #, quietly = TRUE)
library(Rsamtools) #, lib.loc=lib_loc) #, quietly = TRUE)
...
if(runWithNormal){
  ...
  print(names(regionUniqMappedRegions))
